...
def _render_onboarding():
    """Guided mission creation for new users."""
    st.markdown("""
    <style>
        .onboarding-wrapper {
            max-width: 900px;
    ...
    <div class="onboarding-wrapper">
        <div class="onboarding-logo">
            {logo_svg}
            <h1 style='font-size:2rem; margin:5px 0 0 0; letter-spacing: 5px; font-weight: 300; color: #e0e0e0;'>
                ...
                <span style='font-weight:600; color:#2ecc71;'>I</span>.
            </h1>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if step == 1:
        st.markdown('<div class="step-header"><h2>Step 1: Choose Mission Type</h2></div>', unsafe_allow_html=True)
        ...
        with col1:
            ...
            <div class="option-card">
                ...
                <div class="option-title">Create New Mission</div>
                <div class="option-desc">Start from scratch with custom parameters</div>
            </div>
            """, unsafe_allow_html=True)
            st.markdown("<div style='margin-top: 15px;'></div>", unsafe_allow_html=True)
            if st.button("🚀 Create New Mission", use_container_width=True, type="primary", key="btn_new"):
        ...
        with col2:
            ...
            <div class="option-card">
                <div class="option-icon">🧪</div>
                <div class="option-title">Load Demo Mission</div>
                <div class="option-desc">Pre-configured mission with sample data</div>
            </div>
            """, unsafe_allow_html=True)
            st.markdown("<div style='margin-top: 15px;'></div>", unsafe_allow_html=True)
            if st.button("🧪 Load Demo Mission", use_container_width=True, key="btn_demo"):
                st.session_state.onboarding_mission_type = "demo"
                st.session_state.onboarding_step = 2

        with st.expander("💬 Already have an invitation?"):
            st.markdown("Ask your team lead for an invitation code to join their mission.")
            invite_code = st.text_input("Enter invitation code", key="invite_code")
            if st.button("Join Mission", type="secondary"):
                if invite_code:
                    st.warning("⏳ Invitation system coming soon!")
                else:
                    st.warning("⚠️ Please enter an invitation code")

    elif step == 2:
        m_type = st.session_state.get("onboarding_mission_type", "new")
        is_demo = m_type == "demo"
        
        st.markdown('<div class="step-header"><h2>Step 2: Mission Details</h2></div>', unsafe_allow_html=True)
        ...
        with st.container():
            ...
            with col1:
                mission_name = st.text_input("Mission Name", placeholder="My Satellite Mission", key="onboarding_name")
            with col2:
                mission_framework = st.selectbox("Framework", ["ESA", "NASA"], key="onboarding_fw")
            
            mission_desc = st.text_area("Description", placeholder="Describe your mission objectives and purpose...", key="onboarding_desc", height=4)
            
            if is_demo:
                mission_phase = st.selectbox("Starting Phase", ["A", "B1", "B2", "C", "D"], index=2, key="onboarding_phase")
            else:
                mission_phase = "0"
            ...
            st.markdown('<div class="form-section">', unsafe_allow_html=True)
            st.markdown('<div class="form-section-title">🛰️ Orbital Parameters</div>', unsafe_allow_html=True)
            
            c3, c4 = st.columns(2)
            with c3:
                orb_alt = st.number_input("Orbit Altitude (km)", 400, 36000, 550, 50, key="onboarding_alt")
                orb_inc = st.number_input("Orbit Inclination (°)", 0, 180, 97, 1, key="onboarding_inc")
            with c4:
                orb_mass = st.number_input("Dry Mass (kg)", 10, 5000, 150, 10, key="onboarding_mass")
                propellant_kg = st.number_input("Propellant (kg)", 0, 1000, 25 if is_demo else 0, 5, key="onboarding_prop")
            
            st.markdown('</div>', unsafe_allow_html=True)

        c5, c6 = st.columns([1, 3])
        with c5:
            if st.button("⬅️ Back", use_container_width=True, key="btn_back2"):
                st.session_state.onboarding_step = 1
                st.rerun()
        with c6:
            if st.button("Continue ➡️", use_container_width=True, type="primary", key="btn_next2"):
                if not mission_name:
                    st.error("⚠️ Mission name is required")
                else:
                    st.session_state.onboarding_mission_name = mission_name
                    st.session_state.onboarding_mission_desc = mission_desc
                    st.session_state.onboarding_mission_fw = mission_framework
                    st.session_state.onboarding_mission_phase = mission_phase
                    st.session_state.onboarding_orb_alt = orb_alt
                    st.session_state.onboarding_orb_inc = orb_inc
                    st.session_state.onboarding_orb_mass = orb_mass
                    st.session_state.onboarding_propellant = propellant_kg
        ...
        st.markdown('<div class="step-header"><h2>Step 3: Add Team (Optional)</h2></div>', unsafe_allow_html=True)
        
        st.info("💡 You can add team members now or skip this step.")
        
        if "onboarding_team" not in st.session_state:
            st.session_state.onboarding_team = []
        
        team = st.session_state.onboarding_team
        
        st.markdown("<div style='margin-top: 30px;'></div>", unsafe_allow_html=True)
        
        if st.button("+ Add Team Member", key="add_team_member"):
            team.append({"name": "", "role": "SE", "email": ""})
            st.rerun()
        
        if team:
            st.markdown("""
            <div style="background: linear-gradient(90deg, #1e3a5f 0%, #2d5a87 100%); padding: 15px 20px; border-radius: 8px; margin-bottom: 15px;">
                <div style="display: flex; gap: 20px; color: white; font-weight: 600; font-size: 0.95rem;">
                    <div style="flex: 2;">👤 <span style="border-bottom: 2px solid #5cb85c;">Full Name</span></div>
                    <div style="flex: 1.5;">⚙️ <span style="border-bottom: 2px solid #5cb85c;">Role</span></div>
                    <div style="flex: 1.5;">📧 <span style="border-bottom: 2px solid #5cb85c;">Email</span></div>
            ...
            for i, member in enumerate(team):
                ...
                with c1:
                    st.text_input("Full Name", value=member.get("name", ""), key=f"team_name_{i}", placeholder="e.g. Mario Rossi")
                with c2:
                    role_keys = list(ROLES.keys())
                    role_idx = role_keys.index(member.get("role", "SE")) if member.get("role", "SE") in role_keys else 0
                    st.selectbox("Role", role_keys, index=role_idx, key=f"team_role_{i}", format_func=lambda r: ROLES[r]["name"])
                with c3:
                    st.text_input("Email", value=member.get("email", ""), key=f"team_email_{i}", placeholder="collega@uniud.it")
                with c4:
                    st.markdown("<div style='margin-top: 28px;'></div>", unsafe_allow_html=True)
                    if st.button("❌", key=f"team_del_{i}"):
                        team.pop(i)
                        st.rerun()
            st.markdown("---")
        ...
        with c7:
            if st.button("⬅️ Back", use_container_width=True, key="btn_back3"):
                st.session_state.onboarding_step = 2
                st.rerun()
        with c8:
            if st.button("Skip for now", key="onboarding_skip"):
                st.session_state.onboarding_finish = True
                st.session_state.onboarding_step = 4
                st.rerun()
        with c9:
            if st.button("Create Mission ✅", type="primary", key="onboarding_finish"):
                st.session_state.onboarding_finish = True
                st.session_state.onboarding_step = 4
                st.rerun()

    elif step == 4:
        m_name = st.session_state.get("onboarding_mission_name", "My Mission")
        m_desc = st.session_state.get("onboarding_mission_desc", "")
        m_fw = st.session_state.get("onboarding_mission_fw", "ESA")
        m_type = st.session_state.get("onboarding_mission_type", "new")
        is_demo = m_type == "demo"
        
        m_phase = st.session_state.get("onboarding_mission_phase", "0")
        m_alt = st.session_state.get("onboarding_orb_alt", 550)
        m_inc = st.session_state.get("onboarding_orb_inc", 97)
        m_mass = st.session_state.get("onboarding_orb_mass", 150)
        m_prop = st.session_state.get("onboarding_propellant", 25)
        
        # Create mission in DB if enforced
        if DB_ENFORCED:
            db_mission = add_mission(
                ...
                m_phase,
                m_fw,
                owner_user_id=st.session_state.get("user", {}).get("id"),
            ...
            if db_mission:
                ...
                if is_demo:
                    seed_demo_mission(mid, m_name, m_fw)
            else:
                st.error("Failed to create mission in DB.")
                st.stop()
        else:
            ...
            new_mission = {
                ...
                "orb_mass": m_mass,
                "propellant_kg": m_prop,
                "team_members": st.session_state.get("onboarding_team", []),
            }
            if is_demo:
                new_mission.update(_default_mission_data(m_name, with_demo=True))
            else:
                new_mission.update(_default_mission_data(m_name, with_demo=False))
        ...
        if user_id:
            existing_ids = [m.get("id") for m in new_mission.get("team_members", [])]
            if user_id not in existing_ids:
                new_mission["team_members"].append({
                    "id": user_id,
                    "name": st.session_state.get("user", {}).get("full_name", "Me"),
                    "role": "ADMIN",
        ...
        st.balloons()
        st.success(f"Mission '{m_name}' created successfully!")
        st.rerun()
...
def _check_onboarding():
    ...
    # Force onboarding for users who just logged in via Supabase
    ...
    active_mid = st.session_state.get("active_mission_id")
    if not active_mid or active_mid not in missions:
        return True
