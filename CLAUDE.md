...
## Mission Analysis v2 (2026-07-20, sera) — propagatori + interconnessione

Seconda iterazione (workflow 7 agenti, 9 finding, 5 majors fixati) per "staccare Apside" e collegare l'analisi ai dati di missione:

- **Propagatori**: `two_body`/`j2` ora **ellittici veri** (solve_kepler Newton vettorizzato, robusto a e~0.9; argp/ν onorati; rate secolari J2 su p=a(1-e²); back-compat e=0 verificata a 8.8e-11 km vs codice circolare). Nuovo **`j2_drag`**: King-Hele + atmosfera esponenziale Vallado Table 8-4 (28 bande, `atmosphere_density()` esposta), fattore co-rotazione — decay 30d validati vs riferimenti GMAT (400km: 11.29 vs 11.4; 500/75°: 2.04 vs 1.9; retrogrado>progrado ✓); `estimate_lifetime()` (25-year rule, per e>0 quadratura di Gauss 512 nodi lungo l'anomalia eccentrica che campiona la densità fino al perigeo — il fix del finding maggiore: la versione SMA-only dava lifetime ∞ su un GTO). Nuovo **`sgp4`** da TLE (dipendenza `sgp4==2.27` pinnata; TEME≈ECI documentato Phase A/B; UserWarning se TLE >30gg dall'epoca; elementi Kepleriani della spec derivati dalla riga 2 del TLE). Worst-case link ora dall'**apogeo della traiettoria propagata** (Molniya: slant 24701→44145 km, era ottimista di 5 dB). Critical inclination 63.43°→drift argp ~0 ✓, SSO RAAN +0.982°/day ✓.
- **Interconnessione**: "Load from mission budget" (loads ← `equip_budgets`×`operating_modes`, helper puro `loads_from_equipment()`), massa j2_drag ← rollup, prefill ecc/RAAN/AOP ← fase, **`msim_saved_spec` in `_MISSION_DATA_KEYS`** (spec per-missione, sopravvive al cambio); fix majors: `_load_mission` DB path ora ripristina-o-cancella le chiavi assenti dal payload (prima la spec "leakava" tra missioni) e `_MSIM_TRANSIENT_KEYS` (result/spec/lifetime/loads_df/gs_df) poppate a ogni switch. Bottone "↺ Reset inputs" per ri-sincronizzare coi default di fase.
- **Stitch Integrations**: expander GS con multiselect **"Add from ESA network"** ← `COMMON_GROUND_STATIONS` di `gmat.py` (condivise con Integrations); export **"Download GMAT .script"** nei risultati dalla stessa spec (skip per sgp4). Nuovo taglio deciso per Integrations: piano di verifica/compliance ad alta fedeltà (nativo=itera, GMAT/DRAMA/DAS=verifica), riordino grosso rimandato allo split di Matteo.
- **Test**: `tests/unit/test_propagators.py` 16 test (11 + 5 regression della review). Suite piena **84 passed**. Collaudo live: 3 GS (Padova+Svalbard+Malindi), 42 passes con Svalbard dominante da SSO ✓, worst-case link ricalcolato a min-elev 5° (13.6 dB) ✓.
- **Hotfix hot-reload**: spec salvate da build precedenti del motore (senza `drag_coefficient`) crashavano l'export GMAT → guardia di schema che scarta spec/result stantii con invito a rilanciare + `getattr` difensivo.
